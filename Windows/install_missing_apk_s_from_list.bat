...
set "LIST_FILE=%CD%\lists\packages_to_install.txt"
...
for %%F in ("%APK_DIR%\*.apk") do (
    ...
    if /I "%%~xF"==".apk" (
        ...
        for /f "tokens=2 delims=='" %%A in ('aapt dump badging "%%F" ^| findstr /C:"package: name="') do (
            set "PKG_NAME=%%A"
        )
    )
	:: Lecture de .apks
    if /I "%%~xF"==".apks" (
		...
		for /f "delims=" %%A in ('dir /b /a:-d "%TMP_DIR%\*.apk" ^| sort') do (
			if not defined PKG_NAME (
				:: Extracton du package name via aapt
				for /f "tokens=2 delims=='" %%P in ('aapt dump badging "%TMP_DIR%\%%A" ^| findstr /C:"package: name="') do (
					set "PKG_NAME=%%P"
				)
			)
        )
